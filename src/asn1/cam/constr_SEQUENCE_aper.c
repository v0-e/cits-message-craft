...
SEQUENCE_decode_aper(const asn_codec_ctx_t *opt_codec_ctx,
    ...
    for(edx = 0; edx < td->elements_count; edx++) {
        ...
        if(padding > 0)
            ASN_DEBUG("For element %s,offset= %lld Padding bits = %d", td->name, pd->moved, padding);
        ...
        if(edx != (td->elements_count-1))
            ...
            if(specs->roms_count && (padding > 0))
                ASN_DEBUG(">>>>> not skipping padding of %d bits for element:%lld out of %d", padding, edx, td->elements_count);
            else
                per_get_few_bits(pd, padding);
        }
