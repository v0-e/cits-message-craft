...
INTEGER_encode_uper(const asn_TYPE_descriptor_t *td,
    ...
    if(ct) {
        ...
        if(specs && specs->field_unsigned) {
            ...
            ASN_DEBUG("Value %llu (%02x/%" ASN_PRI_SIZE ") lb %llu ub %llu %s",
            ...
            ASN_DEBUG("Value %lld (%02x/%" ASN_PRI_SIZE ") lb %lld ub %lld %s",
    ...
    if(ct && ct->range_bits >= 0) {
        ...
        if(specs && specs->field_unsigned) {
            ...
            || (value.u > (uintmax_t)ct->upper_bound)) {
                ASN_DEBUG("Value %llu to-be-encoded is outside the bounds [%llu, %llu]!",
        ...
        ASN_DEBUG("Encoding integer %llu with range %d bits",
                  v, ct->range_bits);
