...
uper_open_type_get_simple(const asn_codec_ctx_t *ctx,
	...
	ASN_DEBUG("Getting open type %s encoded in %lld bytes", td->name,
		(long long)bufLen);
...
uper_ugot_refill(asn_per_data_t *pd) {
	...
	ASN_DEBUG("REFILLING pd->moved=%lld, oldpd->moved=%lld",
		(long long)pd->moved, (long long)oldpd->moved);
	...
	if(arg->unclaimed) {
		...
		ASN_DEBUG("UNCLAIMED <- return from (pd->moved=%lld)",
			(long long)pd->moved);
		return 0;
	}
	...
	ASN_DEBUG("Open type LENGTH %lld bytes at off %lld, repeat %lld",
		(long long)next_chunk_bytes, (long long)oldpd->moved, (long long)arg->repeat);
	...
	if(avail >= next_chunk_bits) {
		...
		arg->unclaimed = 0;
		ASN_DEBUG("!+Parent frame %lld bits, alloting %lld [%lld..%lld] (%lld)",
			(long long)next_chunk_bits, (long long)oldpd->moved,
			(long long)oldpd->nboff, (long long)oldpd->nbits,
			(long long)(oldpd->nbits - oldpd->nboff));
	} else {
		pd->nbits = oldpd->nbits;
		arg->unclaimed = next_chunk_bits - avail;
		ASN_DEBUG("!-Parent frame %lld, require %lld, will claim %lld",
			(long long)avail, (long long)next_chunk_bits,
			(long long)arg->unclaimed);
	}
	pd->buffer = oldpd->buffer;
