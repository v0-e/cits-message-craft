...
OCTET_STRING_compare(const asn_TYPE_descriptor_t *td, const void *aptr,
    ...
    if(a && b) {
        ...
        if(ret == 0) {
            /* Figure out which string with equal prefixes is long longer. */
            if(a->size < b->size) {
                return -1;
...
OCTET_STRING_per_get_characters(asn_per_data_t *po, uint8_t *buf,
                                ...
                                long long lb, long long ub, const asn_per_constraints_t *pc) {
...
OCTET_STRING_per_put_characters(asn_per_outp_t *po, const uint8_t *buf,
    ...
    for(ub -= lb; buf < end; buf += bpc) {
        ...
        ch = value - lb;
        if(ch < 0 || ch > ub) {
            ASN_DEBUG("Character %d (0x%02x) is out of range (%lld..%lld)", *buf,
                      value, lb, ub + lb);
            return -1;
        }
