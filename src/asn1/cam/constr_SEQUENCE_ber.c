...
SEQUENCE_decode_ber(const asn_codec_ctx_t *opt_codec_ctx,
    ...
    case 0:
        ...
        ASN_DEBUG("Structure consumes %lld bytes, buffer %lld",
                  (long long)ctx->left, (long long)size);
    ...
    case 1:
        ...
        for(edx = ((size_t)ctx->step >> 1); edx < td->elements_count;
            ...
            ASN_DEBUG("Current tag in %s SEQUENCE for element %" ASN_PRI_SIZE " "
                      "(%s) is %s encoded in %d bytes, of frame %lld",
                      td->name, edx, elements[edx].name,
                      ber_tlv_tag_string(tlv_tag), (int)tag_len, (long long)LEFT);
            switch(tag_len) {
        ...
        microphase2:
            ...
            case RC_WMORE: /* More data expected */
                ...
                ASN_DEBUG("Size violation (c->l=%lld <= s=%lld)",
                          (long long)ctx->left, (long long)size);
                /* Fall through */
    ...
    case 4:  /* only 00's expected */
        ...
        ASN_DEBUG("SEQUENCE %s Leftover: %lld, size = %lld",
                  td->name, (long long)ctx->left, (long long)size);
...
SEQUENCE_encode_der(const asn_TYPE_descriptor_t *td, const void *sptr,
    ...
    for(edx = 0; edx < td->elements_count; edx++) {
        ...
        ASN_DEBUG("Member %" ASN_PRI_SIZE " %s estimated %lld bytes",
                  edx, elm->name, (long long)erval.encoded);
    }
    ...
    ASN_DEBUG("Wrote tags: %lld (+%lld)", (long long)ret, (long long)computed_size);
    if(ret == -1)
        ASN__ENCODE_FAILED;
    ...
    for(edx = 0; edx < td->elements_count; edx++) {
        ...
        ASN_DEBUG("Member %" ASN_PRI_SIZE " %s of SEQUENCE %s encoded in %lld bytes",
                  edx, elm->name, td->name, (long long)tmperval.encoded);
    }
