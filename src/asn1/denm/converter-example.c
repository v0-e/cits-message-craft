...
#define    _ASX(x)    _AXS(x)
        ...
        "  -b <size>    Set the i/o buffer size (default is %lld)\n"
...
#endif
        , (long long)suggested_bufsize, ASN__DEFAULT_STACK_MAX);
        exit(EX_USAGE);
    }
...
buffer_dump() {
    ...
    if(DynamicBuffer.unbits) {
        ...
        fprintf(stderr, " %lld\n", (long long)DynamicBuffer.length);
    }
}
...
buffer_shift_left(size_t offset, int bits) {
    ...
    DEBUG("Shifting left %d bits off %lld (o=%lld, u=%lld, l=%lld)",
        bits, (long long)offset,
        (long long)DynamicBuffer.offset,
        (long long)DynamicBuffer.unbits,
        (long long)DynamicBuffer.length);
...
static void add_bytes_to_buffer(const void *data2add, size_t bytes) {
    ...
    if(DynamicBuffer.allocated
       >= (DynamicBuffer.offset + DynamicBuffer.length + bytes)) {
        ...
        DEBUG("\tContents shifted by %lld", DynamicBuffer.offset);
        ...
        DEBUG("\tBuffer reallocated to %lld (%d time)",
...
data_decode_from_file(enum asn_transfer_syntax isyntax, asn_TYPE_descriptor_t *pduType, FILE *file, const char *name, ssize_t suggested_bufsize, int on_first_pdu) {
    ...
    for(tolerate_eof = 1;    /* Allow EOF first time buffer is non-empty */
        ...
        case RC_OK:
            ...
            DEBUG("RC_OK, finishing up with %lld+%d",
                (long long)rval.consumed, ecbits);
            return structure;
        case RC_WMORE:
            DEBUG("RC_WMORE, continuing read=%lld, cons=%lld "
                " with %lld..%lld-%lld..%lld",
                (long long)rd,
                (long long)rval.consumed,
                (long long)DynamicBuffer.offset,
                (long long)DynamicBuffer.length,
                (long long)DynamicBuffer.unbits,
                (long long)DynamicBuffer.allocated);
            if(!rd) tolerate_eof--;
            continue;
        case RC_FAIL:
            break;
    ...
    || new_offset - old_offset > ((isyntax == ATS_BASIC_XER)?sizeof("\r\n")-1:0)
        ...
        DEBUG("ofp %d, no=%lld, oo=%lld, dbl=%lld",
            on_first_pdu, (long long)new_offset, (long long)old_offset,
            (long long)DynamicBuffer.length);
        fprintf(stderr, "%s: "
            "Decode failed past byte %lld: %s\n",
            name, (long long)new_offset,
            (rval.code == RC_WMORE)
