...
BIT_STRING_decode_uper(const asn_codec_ctx_t *opt_codec_ctx,
    ...
    /* X.691, #16.7: long long fixed length encoding (up to 64K octets) */
    if(csiz->effective_bits == 0) {
        int ret;
        ASN_DEBUG("Encoding BIT STRING size %lld", csiz->upper_bound);
    ...
    do {
        ...
        ASN_DEBUG("Got PER length eb %lld, len %lld, %s (%s)",
            (long long)csiz->effective_bits, (long long)raw_len,
...
BIT_STRING_encode_uper(const asn_TYPE_descriptor_t *td,
    ...
    if(csiz->effective_bits >= 0 && !inext) {
        ...
        ASN_DEBUG(
            "Encoding %" ASN_PRI_SIZE " bytes (%lld), length (in %d bits) trailer %d; actual "
            "value %" ASN_PRI_SSIZE "",
