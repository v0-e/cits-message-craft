 ...
 */
static uint32_t
OCTET_STRING__random_char(unsigned long long lb, unsigned long long ub) {
    assert(lb <= ub);
...
OCTET_STRING_random_fill(const asn_TYPE_descriptor_t *td, void **sptr,
    ...
    unsigned long long clb = 0;  /* Lower bound on char */
    unsigned long long cub = 255;  /* Higher bound on char value */
    uint8_t *buf;
    uint8_t *bend;
    uint8_t *b;
    size_t rnd_len;
...
OCTET_STRING_random_length_constrained(
    ...
    if(constraints->per_constraints) {
        ...
        if(pc->flags & APC_CONSTRAINED) {
            long long suggested_upper_bound = pc->upper_bound < (ssize_t)max_length
                                             ? pc->upper_bound
                                             : (ssize_t)max_length;
