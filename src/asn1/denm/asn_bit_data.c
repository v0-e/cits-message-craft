...
asn_put_few_bits(asn_bit_outp_t *po, uint32_t bits, int obits) {
	...
	if(po->nboff + obits > po->nbits) {
		...
		ASN_DEBUG("[PER output %lld complete + %lld]",
			(long long)complete_bytes, (long long)po->flushed_bytes);
	...
	ASN_DEBUG("[PER out %u/%x => %02x buf+%lld]",
		(int)bits, (int)bits, buf[0],
		(long long)(po->buffer - po->tmpspace));
