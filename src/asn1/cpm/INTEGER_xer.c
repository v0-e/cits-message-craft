...
INTEGER__xer_body_decode(const asn_TYPE_descriptor_t *td, void *sptr,
    ...
    if(chunk_size)
        ASN_DEBUG("INTEGER body %lld 0x%2x..0x%2x",
                  (long long)chunk_size, *lstart, lstop[-1]);
    ...
    for(lp = lstart; lp < lstop; lp++) {
        ...
        case 0x3a:  /* ':' */
            if(state == ST_HEXCOLON) {
                ...
                ASN_DEBUG("state %d at %lld", state, (long long)(lp - lstart));
                break;
            }
        /* [A-Fa-f] */
        ...
        ASN_DEBUG("INTEGER :: Found non-numeric 0x%2x at %lld",
                  lv, (long long)(lp - lstart));
        state = ST_UNEXPECTED;
        break;
    }
    ...
    case ST_DIGITS_TRAILSPACE:
        ...
        case ASN_STRTOX_OK:
            if(specs && specs->field_unsigned && (uintmax_t) dec_value <= ULLONG_MAX) {
                break;
            } else if(dec_value >= LLONG_MIN && dec_value <= LLONG_MAX) {
                break;
            } else {
                /*
                 * We model INTEGER on long long for XER,
                 * to avoid rewriting all the tests at once.
                 */
                ASN_DEBUG("INTEGER exceeds long long range");
            }
            /* Fall through */
