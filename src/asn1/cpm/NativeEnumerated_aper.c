...
NativeEnumerated_decode_aper(const asn_codec_ctx_t *opt_codec_ctx,
    ...
    if(ct && ct->range_bits >= 0) {
        ...
        //    ASN__DECODE_FAILED;
        if(value >= specs->map_count) {
            ASN_DEBUG("Decoded unknown index value %s = %lld", td->name, value);
    ...
    *native = specs->value2enum[value].nat_value;
    ASN_DEBUG("Decoded %s = %lld", td->name, *native);
...
NativeEnumerated_encode_aper(const asn_TYPE_descriptor_t *td,
    ...
    if(!kf) {
        ASN_DEBUG("No element corresponds to %lld", native);
        ASN__ENCODE_FAILED;
    }
     ...
     * X.691, #10.6: normally small non-negative whole number;
     */
    ASN_DEBUG("value = %lld, ext = %d, inext = %d, res = %lld",
