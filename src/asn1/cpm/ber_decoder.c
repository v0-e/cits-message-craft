...
ber_check_tags(const asn_codec_ctx_t *opt_codec_ctx,
	...
	tagno = step	/* Continuing where left previously */
		+ (tag_mode==1?-1:0)
		;
	ASN_DEBUG("ber_check_tags(%s, size=%lld, tm=%d, step=%d, tagno=%d)",
		td->name, (long long)size, tag_mode, step, tagno);
	...
	if(tag_mode == 0 && tagno == (int)td->tags_count) {
		...
		ASN_DEBUG("Advancing %lld in ANY case",
			(long long)(tag_len + len_len));
	...
	for((void)tagno; tagno < (int)td->tags_count; tagno++, step++) {
		 ...
		 */
		tag_len = ber_fetch_tag(ptr, size, &tlv_tag);
			ASN_DEBUG("Fetching tag from {%p,%lld}: "
				"len %lld, step %d, tagno %d got %s",
				ptr, (long long)size,
				(long long)tag_len, step, tagno,
		...
		len_len = ber_fetch_length(tlv_constr,
			(const char *)ptr + tag_len, size - tag_len, &tlv_len);
		ASN_DEBUG("Fetching len = %lld", (long long)len_len);
		switch(len_len) {
		case -1: RETURN(RC_FAIL);
		case 0: RETURN(RC_WMORE);
		}
		...
		if(limit_len == -1) {
			 ...
			 */
			ASN_DEBUG("Outer TLV is %lld and inner is %lld",
				(long long)limit_len, (long long)tlv_len);
			RETURN(RC_FAIL);
		}
