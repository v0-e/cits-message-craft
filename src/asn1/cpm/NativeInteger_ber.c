...
NativeInteger_decode_ber(const asn_codec_ctx_t *opt_codec_ctx,
    ...
    long long *native = (long long *)*nint_ptr;
    asn_dec_rval_t rval;
     ...
     */
    if(native == NULL) {
        native = (long long *)(*nint_ptr = CALLOC(1, sizeof(*native)));
        if(native == NULL) {
            rval.code = RC_FAIL;
            rval.consumed = 0;
            return rval;
    ...
    ASN_DEBUG("Took %lld/%lld bytes to encode %s (%lld)",
              (long long)rval.consumed, (long long)length, td->name, (long long)*native);
...
NativeInteger_encode_der(const asn_TYPE_descriptor_t *sd, const void *ptr,
    ...
    unsigned long long native = *(const unsigned long long *)ptr; /* Disable sign ext. */
